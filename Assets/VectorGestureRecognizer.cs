...
{
    public List<RecognizeOutput> RecognizeGesture(VectorGesture gesture,List<VectorGesture> database)
    {
        List<RecognizeOutput> output = new List<RecognizeOutput>();
        foreach (var gestureFromDatabase in database)
        {
            float propability = CompareGesture(gesture, gestureFromDatabase);
